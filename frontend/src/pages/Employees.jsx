...
export default function Employees() {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this employee?")) {
      try {
        await api.delete(`/delete/employees/details/${id}`, {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        });
          ...
          <table className="min-w-full bg-white shadow rounded-lg overflow-hidden">
            ...
            <tbody className="text-gray-700 text-sm">
              {employees.map((e, index) => (
                ...
                >
                  <td className="px-6 py-4 text-center">{e.employee_id}</td>
                  <td className="px-6 py-4 text-center">{e.name}</td>
                  <td className="px-6 py-4 text-center">{e.gender}</td>
                  <td className="px-6 py-4 text-center">{e.age}</td>
                  <td className="px-6 py-4 text-center">{e.designation}</td>
                  <td className="px-6 py-4 text-center">{e.department}</td>
                  <td className="px-6 py-4 text-center">{e.mobile_no}</td>
                  <td className="px-6 py-4 text-center">{e.email}</td>
                  ...
                  <td className="px-6 py-4 text-center">
                    ...
                    <button
                onClick={() => window.location.href = `/edit-employee/${e._id}`}
